...
.controller('ExploreCtrl', ["$scope", "$stateParams", "$http", "PROXY", "$firebaseArray", "$window", "$timeout", function($scope, $stateParams, $http, PROXY, $firebaseArray, $window, $timeout){

  $scope.$on("$ionicView.enter", function (e) {

      $timeout(function() {
        angular.element(exploreview).removeClass('loading');
      }, 2000)
      $scope.userId = window.localStorage.getItem("userId");
      // On page load, run ajax call
      $scope.runAjaxCall = function () {
        $http({
          method: 'GET',
          url: PROXY.url + "/beer/random/?key=124796ba126c92f04f87e154a597c112&format=json&hasLabels=Y&withBreweries=Y",
          cache: false
      ...
      $scope.saveToWishlist = function () {
        ...
        // $scope.runAjaxCall(); Caching is preventing call from running. Reload page instead.
